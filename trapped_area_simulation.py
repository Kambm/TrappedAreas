...
def simulate(nphases=151, Lmax=5, klambd=10, runs=1000, gammas=np.logspace(3,-1,base=0.1), dir_name='yvals'):
	"""
...
def compute_areas(dirname='yvals', data_fname='new_data' runs=1000, gammas=np.logspace(3,-1,base=0.1)):
	"""
	...
	"""
	for i in range(len(gammas)):
		yval_list.append(np.stack([np.load('yvals/yvals%d/yvals%d.npy' %(i,j)) for j in range(runs)],axis=0))
